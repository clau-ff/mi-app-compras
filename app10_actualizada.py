...
if nombre_producto:
    ...
    df_trans['texto_busqueda_norm'] = (df_trans['notes'].fillna('') + ' ' + (df_trans['itemName'].fillna('') if 'itemName' in df_trans.columns else '')).apply(normalizar)
    if solo_con_boleta == "Sí":
        ...
        if 'NewSplitTransactionID' in df_trans.columns:
            # Splits que tienen boleta (al menos una de sus partes tiene imagen)
            split_ids_con_boleta = set(
                df_trans.loc[
                    df_trans['transactionsTableID'].isin(trans_ids_con_boleta) & df_trans['NewSplitTransactionID'].notna(),
            ...
            df_base = df_trans[
                (
                    ...
                    df_trans['transactionsTableID'].isin(trans_ids_con_boleta)
                )
            ].copy()
        else:
